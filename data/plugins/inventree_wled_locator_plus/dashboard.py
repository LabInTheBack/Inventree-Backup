...
def build_dashboard(plugin, params) -> dict:
    ...
    for location in locations:
        ...
        if row["controller"] and row["leds"]:
            for led in row["leds"]:
                led_usage.setdefault((row["controller"], led), []).append(row)

    conflict_keys = {key for key, rows in led_usage.items() if len(rows) > 1}

    for row in all_rows:
        row["conflict"] = (
            any((row["controller"], led) in conflict_keys for led in row["leds"])
            if row["controller"]
            else False
        )
...
def row_matches_filters(row: dict, filters: dict, root_ids: set[int] | None) -> bool:
    ...
    if filters["led"]:
        try:
            if int(filters["led"]) - 1 not in row["leds"]:
                return False
        except ValueError:
            return False
    ...
    if text:
        searchable = (
            ...
            f"{display_leds(row['leds'])} {row.get('component_text', '')} "
...
def display_leds(leds) -> str:
    """Return comma-separated 1-based LED display text."""
    if not isinstance(leds, list):
        return display_led(leds)

    return ", ".join(filter(None, (display_led(led) for led in leds)))

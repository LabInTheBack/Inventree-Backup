...
function locationAssignment(entry, options) {
    ...
    const currentLedValue = Array.isArray(entry?.leds) && entry.leds.length
        ? entry.leds
        : (entry?.led ?? "");
...
function bindAssignmentForms(container, result, options) {
    container.querySelectorAll("[data-location-assignment]").forEach((assignment) => {
        ...
        testButton.addEventListener("click", async () => {
            const leds = getLedValues(form, options.maxLed);
            if (!leds.length) {
                result.textContent = ledInputMessage(form, options.maxLed);
                return;
            }

            await runJsonRequest(testLedUrl(options.testControllerUrlBase || options.testUrlBase, form, leds), testButton, result, "Testing LED...", summarizePayload);
        });
        ...
        form.addEventListener("submit", async (event) => {
            ...
            const payload = await runJsonRequest(
                registerLedUrl(options, locationId, form, leds),
            ...
            if (payload?.success) {
                const ledCell = assignment.closest("tr")?.querySelector("[data-led-cell]");
                if (ledCell) {
                    ledCell.textContent = compactIndicatorText(payload);
                }
            }
        });
    });

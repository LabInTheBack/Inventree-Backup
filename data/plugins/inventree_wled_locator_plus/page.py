...
from .dashboard import build_dashboard, display_led, display_leds, split_location_path
...
<body>
  <main>
    ...
    <section>
      ...
      <div class="bulk-bar">
        ...
        <label>LED(s)
          <input data-bulk-led type="text" inputmode="numeric" placeholder="15 or 15,16,17">
        </label>
  ...
  <script>
    (() => {{
      ...
      function rowLedValues(row) {{
        return String(row.dataset.leds || row.dataset.led || "")
          .split(",")
          .map((item) => item.trim())
          .filter(Boolean);
      }}

      function rowHasLedMapping(row) {{
        return row.dataset.controller && row.dataset.controller !== "-" && rowLedValues(row).length > 0;
      }}

      function parseLedDisplayList(value, maxLed) {{
        ...
        for (const part of parts) {{
          ...
          if (!Number.isInteger(number) || number < 1 || (maxLed > 0 && number > maxLed)) {{
            return null;
          }}
          if (!leds.includes(number)) {{
            leds.push(number);
          }}
        }}

        return leds.length ? leds : null;
      }}
      ...
      function applyLedFilter(controller, led) {{
        ...
        let shown = 0;
        rows.forEach((row) => {{
          const match = row.dataset.controller === controller && rowLedValues(row).includes(led);
          row.hidden = !match;
          if (match) shown += 1;
        }});
      ...
      async function reassignRows(targetRows) {{
        ...
        const ledValues = parseLedDisplayList(bulkLed ? bulkLed.value : "", targetMax);
        ...
        if (!ledValues) {{
          setStatus("Enter target LED number(s), for example 15 or 15,16,17.", true);
          return;
        }}

        if (rowsToAssign.length > 1 && ledValues.length > 1) {{
          setStatus("Multiple LED numbers can be assigned when one Stock Location row is selected. For multiple rows, enter one start LED.", true);
          return;
        }}

        const lastLed = ledValues[0] + rowsToAssign.length - 1;
        if (rowsToAssign.length > 1 && targetMax > 0 && lastLed > targetMax) {{
          setStatus(`Cannot reassign: selected rows need LEDs ${{ledValues[0]}}-${{lastLed}}, but ${{targetController}} has ${{targetMax}} LEDs.`, true);
          return;
        }}

        const targetText = rowsToAssign.length === 1
          ? `LED${{ledValues.length === 1 ? "" : "s"}} ${{ledValues.join(", ")}}`
          : `LEDs ${{ledValues[0]}}-${{lastLed}}`;
        const confirmed = window.confirm(
          rowsToAssign.length === 1
            ? `Reassign selected Stock Location to ${{targetController}} ${{targetText}}?`
            : `Reassign ${{rowsToAssign.length}} selected Stock Locations to ${{targetController}} ${{targetText}}?`
        );
        if (!confirmed) {{
          return;
        }}
        ...
        for (const [index, row] of rowsToAssign.entries()) {{
          const displayLeds = rowsToAssign.length === 1
            ? ledValues
            : [ledValues[0] + index];
          const zeroBasedLeds = displayLeds.map((led) => led - 1);
          await requestJson(`${{window.location.pathname}}register/location/${{row.dataset.locationId}}/controller/${{encodeURIComponent(targetController)}}/leds/${{zeroBasedLeds.join(",")}}/`);
          row.dataset.controller = targetController;
          row.dataset.led = String(displayLeds[0]);
          row.dataset.leds = displayLeds.join(",");
          row.querySelector("[data-controller-cell]").textContent = targetController;
          row.querySelector("[data-led-cell]").textContent = displayLeds.join(", ");
...
def render_assignment_rows(dashboard: dict) -> str:
    ...
    for row in dashboard["rows"]:
        led_text = display_leds(row["leds"])
        first_led_text = display_led(row["led"])
        ...
        if row["controller"] and row["leds"] and not row["invalid"]:
            actions.insert(
                0,
        ...
        rows.append(
            ...
            f"data-led='{escape(first_led_text)}' "
            f"data-leds='{escape(led_text.replace(' ', ''))}' "

...
class LocatorService:
    ...
    def set_location_indicator(self, location: StockLocation, led: int, controller_label: str | None = None) -> dict:
        ...
        led = self.wled.validate_led_index(led, controller["label"])

        self.set_metadata_values(
            location,
            {METADATA_INDICATOR_KEY: {"controller": controller["label"], "led": led}},
        )

        return {"controller": controller["label"], "led": led}
    ...
    def get_mapping_summary(self) -> dict:
        ...
        for location in StockLocation.objects.order_by("pathstring", "name"):
            ...
            if isinstance(indicator, dict):
                ...
                indicator_led = indicator.get("led", None)

                try:
                    indicator_led = int(indicator_led)
                except (TypeError, ValueError):
                    invalid.append({
                        "location": self.get_location_info(location),
                        "reason": "wled_indicator led is not an integer",
                    })
                    continue
    ...
    def reassign_location_mappings(self, source_controller: str, target_controller: str) -> dict:
        ...
        for location in StockLocation.objects.order_by("pathstring", "name"):
            ...
            self.set_metadata_values(
                location,
                {METADATA_INDICATOR_KEY: {"controller": target["label"], "led": led}},
            )
            moved.append({
                "location": self.get_location_info(location),
